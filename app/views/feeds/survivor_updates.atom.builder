atom_feed(:schema_date => "#{Time.now.strftime("%Y-%m-%dT%H:%M:%SZ")}", :root_url => "#{haiti_quake_survivors_url}/feeds/survivor_updates.atom?title=#{@title}") do |feed|
  ...
  for survivor_update in @survivor_updates
    feed.entry(survivor_update, :url => "#{haiti_quake_survivors_url}/survivor_updates/#{survivor_update.id}") do |entry|
