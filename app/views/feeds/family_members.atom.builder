atom_feed(:schema_date => "#{Time.now.strftime("%Y-%m-%dT%H:%M:%SZ")}", :root_url => "#{people_finder_url}/feeds/family_members.atom") do |feed|
  ...
  for family_member in @family_members
    feed.entry(family_member.survivor, :url => "#{people_finder_url}/survivors/#{family_member.survivor.id}") do |entry|
