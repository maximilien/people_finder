atom_feed(:schema_date => "#{Time.now.strftime("%Y-%m-%dT%H:%M:%SZ")}", :root_url => "#{people_finder_url}/feeds/friends.atom") do |feed|
  ...
  for friend in @friends
    feed.entry(friend.survivor, :url => "#{people_finder_url}/survivors/#{friend.survivor.id}") do |entry|
